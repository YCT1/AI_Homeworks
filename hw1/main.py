...
if (player_string == "HUMAN"):
    (collected_toy_count, elapsed_time_step, path_cost) = hamham.start_level_human(PLAYED_LEVEL)
    printed_str = "--- Player Statistics ---\n"
    printed_str += "collected toy count:{}\n".format(collected_toy_count)
    printed_str += "elapsed time step:{}\n".format(elapsed_time_step)
    printed_str += "path cost:{}\n".format(path_cost)
    print(printed_str)
elif (player_string == "BFS"):
    agent = BFSAgent()
    (collected_toy_count, elapsed_time_step, elapsed_solve_time, result, path_cost) = hamham.start_level_computer(PLAYED_LEVEL, agent, 
