...
class DQN:
    ...
    def updateOLD(self):
        ...
        for t in batch.state:
            state.append(t)
        
        for t in batch.action:
            action.append(t)

        for t in batch.reward:
            reward.append(t)

        for t in batch.next_state:
            next_state.append(t)

        state_batch = torch.tensor(state).float().to(device=device)
        action_batch = torch.LongTensor(action).view(-1, 1).long().to(device=device)
        reward_batch = torch.tensor(reward).float().to(device=device)
        non_final_next_states = torch.tensor(next_state).float().to(device=device)


        # Compute Q(s_t, a) - the model computes Q(s_t), then we select the
        # columns of actions taken. These are the actions which would've been taken
        # for each batch state according to policy_net
        state_action_values = self.act_net(state_batch).gather(1, action_batch)
